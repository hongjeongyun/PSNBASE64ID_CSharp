...
public static class PSNIDBASE64
{
    ...
    public static string GetBase64ID(string redirect_url)
    {
        ...
        var token_request = client.PostAsync(TOKEN_URL, new StringContent(body, Encoding.ASCII, "application/x-www-form-urlencoded")).Result;

        if (token_request.StatusCode != HttpStatusCode.OK)
            return $"Request failed with code {token_request.StatusCode} : {token_request.Content}";
        ...
        if (!tokeninfo.ContainsKey("access_token"))
            return $"access_token is missing in response JSON: {tokeninfo}";

        var account_request = client.GetAsync(TOKEN_URL + "/" + HttpUtility.UrlEncode(tokeninfo["access_token"]), HttpCompletionOption.ResponseHeadersRead).Result;
